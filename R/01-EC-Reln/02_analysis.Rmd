...
meta <- data.frame(
  ...
  cluster = sub("_([^_]*_[^_]*)$", "", colnames(sce)),
  bio_rep = gsub("^(?:[^_]+_){2}([^_]+).*", "\\1", colnames(sce)),
  group = c(rep("NLGF", 25), rep("WT", 22)),
  age = c(
    rep("3mo", 7), rep("18mo", 3), rep("3mo", 5), rep("18mo", 10),
    rep("3mo", 6), rep("18mo", 3), rep("3mo", 6), rep("18mo", 7)
  )
)
...
design <- model.matrix(~ 0 + genotype_age)
colnames(design) <- levels(genotype_age)
...
# )
contrast_mat <- makeContrasts(
  "NLGF.WT_3mo" = NLGF_3mo-WT_3mo, "NLGF.WT_18mo" = NLGF_18mo-WT_18mo,
  "NLGF.WT_18mo-3mo" = (NLGF_18mo-WT_18mo) - (NLGF_3mo-WT_3mo), levels = design
)
...
cont_fit <- eBayes(
  contrasts.fit(fit, contrast_mat), robust = TRUE, trend = TRUE
)
results <- decideTests(cont_fit, p.value = 0.05, adjust.method = "none")
vennDiagram(results)
```
...
```{r, fig.height = 30}
results <- topTable(cont_fit, 1, Inf, p.value = 0.01, adjust.method = "none")
...
```{r, fig.height = 30}
results <- topTable(cont_fit, 2, Inf, p.value = 0.01, adjust.method = "none")
mat <- as.matrix(logcounts(sce))
mat <- mat[rownames(mat) %in% rownames(results), ]
rownames(results) <- gene_anno[
  which(gene_anno$ensembl_gene_id %in% rownames(results)),
...
```{r, fig.height = 30}
results <- topTable(cont_fit, 3, Inf, p.value = 0.01, adjust.method = "none")
mat <- as.matrix(logcounts(sce))
mat <- mat[rownames(mat) %in% rownames(results), ]
rownames(results) <- gene_anno[
  which(gene_anno$ensembl_gene_id %in% rownames(results)),
