...
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), show_column_names = FALSE, cluster_rows = FALSE, column_split = 2
)
draw(heatmap)

seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[1])])
...
seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[2])])

ident_names <- "EC Reln"
idents <- vector("list", 2)
for (i in 1:(length(idents) - 1)) {
  idents[[i]] <- rep(ident_names[i], length(column_order(heatmap)[[2]])) # TODO: Fix hack.
}
idents[[2]] <- rep("NA", length(colnames(seurat)[colnames(seurat) %notin% colnames(seurat_sub)]))
