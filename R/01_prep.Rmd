...
seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[1:5])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 4,
  column_names_gp = gpar(fontsize = 4)
...
ident_names <- c("EC Rorb", "CA1 Wfs1", "CA3 Bok", "EC Reln")
idents <- vector("list", 5)
for (i in 1:(length(idents) - 1)) {
  idents[[i]] <- rep(ident_names[i], length(column_order(heatmap)[[i]]))
}
...
seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[1:6])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 6,
  column_names_gp = gpar(fontsize = 4)
)
draw(heatmap)
...
for (i in 1:(length(idents) - 1)) {
  if (i == 1) {
    idents[[i]] <- rep(
      ident_names[i],
      length(column_order(heatmap)[[1]]) + length(column_order(heatmap)[[2]]) +
        length(column_order(heatmap)[[3]])
    )
  } else {
    idents[[i]] <- rep(ident_names[i], length(column_order(heatmap)[[i + 2]]))
  }
}
idents[[5]] <- rep("NA", length(colnames(seurat)[colnames(seurat) %notin% colnames(seurat_sub)]))
...
seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[c(1:4, 6:8)])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 5,
  column_names_gp = gpar(fontsize = 4)
)
draw(heatmap)
...
for (i in 1:(length(idents) - 1)) {
  if (i == 4) {
    idents[[i]] <- rep(
      ident_names[i], length(column_order(heatmap)[[4]]) + length(column_order(heatmap)[[5]])
    )
  } else {
    idents[[i]] <- rep(ident_names[i], length(column_order(heatmap)[[i]]))
  }
