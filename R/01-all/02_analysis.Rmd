...
packages <- c(
  ...
  "edgeR", "RColorBrewer", "DT", "circlize", "factoextra", "pals", "dendextend", "tm", "igraph",
  "qgraph", "GO.db"
)
...
genes <- Reduce(
  intersect,
  list(
    rownames(data_list[[1]]), rownames(data_list[[2]])
  )
)

ident_names <- c("EC_L2_Lat_Reln", "HIP_CA1_Wfs1", "HIP_CA3_Prkcd")
for (i in seq_along(data_list)) {
  print(rds_names[i])
  print(
    SpatialPlot(
...
seurat <- merge(data_list[[1]], c(data_list[[2]], data_list[[3]], data_list[[4]]))
...
if (file.exists(rds) & file.exists(rds2)) {
  ...
  allen_mouse_genes <- read.delim(
    file.path(assets_dir, "gene-sets", "allen-mouse-genes.tsv"), header = FALSE
  )
  # roussarie_genes <- read.delim(
  #   file.path(assets_dir, "gene-sets", "roussarie-genes.tsv"), header = FALSE
  # )
  # ref_genes <- rbind(allen_mouse_genes, roussarie_genes)
  gene_anno <- gene_anno[gene_anno$ensembl_gene_id %in% allen_mouse_genes$V1, ]
...
seurat <- suppressWarnings(SCTransform(seurat, verbose = FALSE))
seurat

# Subset other objects.
# ---------------------
gene_anno <- gene_anno[gene_anno$ensembl_gene_id %in% rownames(seurat), ]
#----------------------
...
if (file.exists(rds)) {
  ...
  sce <- suppressWarnings(
    aggregateAcrossCells(
      sce, colData(sce)[ , c("idents", "samples")],
      use_exprs_values = "counts", BPPARAM = MulticoreParam()
    )
  )
  colnames(sce) <- sce$groups
  sce$idents <- factor(rep(levels(seurat$idents), each = 4), levels = levels(seurat$idents))
  sce$bio_reps <- sub("_.*", "", sce$samples)
  keep <- filterByExpr(counts(sce), group = sce$idents, min.count = 5, min.total.count = 10)
  sce <- sce[keep, ]
  dge <- DGEList(counts(sce))
  dge <- calcNormFactors(dge)
  assay(sce, "logcounts", FALSE) <- cpm(dge, log = TRUE, prior.count = 3)
  rm(dge)
  saveRDS(sce, rds)
}
...
corr_fit <- duplicateCorrelation(gsva, design, block = sce$bio_reps)

# Fit the strictest model on pseudo-bulk GSVA results.
# ----------------------------------------------------
design <- model.matrix(~ 0 + sce$idents)
colnames(design) <- levels(idents)
fit <- lmFit(gsva, design, block = sce$bio_reps, correlation = corr_fit$consensus.correlation)
contrast_mat <- makeContrasts(
  EC_L2_Lat_Reln-HIP_CA1_Wfs1, HIP_CA1_Wfs1-HIP_CA3_Prkcd, levels = design
)
...
# idx <- ifelse(nrow(all) < 400, nrow(all), 400)
# all <- all[1:idx, ]
...
fit <- lmFit(gsva, design, block = sce$bio_reps, correlation = corr_fit$consensus.correlation)
contrast_mat <- makeContrasts(
  EC_L2_Lat_Reln-HIP_CA3_Prkcd, HIP_CA1_Wfs1-HIP_CA3_Prkcd,
  EC_L2_Lat_Reln-HIP_CA1_Wfs1, levels = design
)
cont_fit <- eBayes(contrasts.fit(fit, contrast_mat))
tests <- decideTests(cont_fit, "nestedF", p.value = 0.05)
tests_up <- tests[tests[ , 1] == 1 & tests[ , 2] == 1, ]
...
fit <- lmFit(gsva, design, block = sce$bio_reps, correlation = corr_fit$consensus.correlation)
contrast_mat <- makeContrasts(
  EC_L2_Lat_Reln-HIP_CA3_Prkcd, EC_L2_Lat_Reln-HIP_CA1_Wfs1,
  HIP_CA1_Wfs1-HIP_CA3_Prkcd, levels = design
)
cont_fit <- eBayes(contrasts.fit(fit, contrast_mat))
tests <- decideTests(cont_fit, "nestedF", p.value = 0.05)
tests_up <- tests[tests[ , 1] == 1 & tests[ , 2] == 1, ]
...
```{r}
design <- model.matrix(~ 0 + sce$idents)
corr_fit <- duplicateCorrelation(logcounts(sce), design, block = sce$bio_reps)
...
colnames(design) <- levels(idents)
fit <- lmFit(
  logcounts(sce), design, block = sce$bio_reps, correlation = corr_fit$consensus.correlation
)
contrast_mat <- makeContrasts(
  EC_L2_Lat_Reln-HIP_CA1_Wfs1, HIP_CA1_Wfs1-HIP_CA3_Prkcd, levels = design
)
...
```
# Black
(EC_L2_Lat_EX_Reln-IN_Sst, HIP_CA1_EX_Wfs1-IN_Sst, HIP_CA3_EX_Prkcd-IN_Sst) # All must be DE.
(EC_L2_Lat_EX_Reln-HIP_CA1_EX_Wfs1, HIP_CA1_EX_Wfs1-HIP_CA3_EX_Prkcd) # Must NOT be DE in opposite direction.

# Yellow
(EC_L2_Lat_EX_Reln-IN_Sst, EC_L2_Lat_EX_Reln-HIP_CA3_EX_Prkcd, EC_L2_Lat_EX_Reln-HIP_CA1_EX_Wfs1) # All must be DE.
(HIP_CA1_EX_Wfs1-HIP_CA3_EX_Prkcd, HIP_CA3_EX_Prkcd-IN_Sst) # Must NOT be DE in opposite direction.

# Orange
(EC_L2_Lat_EX_Reln-IN_Sst, EC_L2_Lat_EX_Reln-HIP_CA3_EX_Prkcd, HIP_CA1_EX_Wfs1-IN_Sst, HIP_CA1_EX_Wfs1-HIP_CA3_EX_Prkcd) # All must be DE.
(EC_L2_Lat_EX_Reln-HIP_CA1_EX_Wfs1, HIP_CA3_EX_Prkcd-IN_Sst) # Must NOT be DE in opposite direction.

# Red
(EC_L2_Lat_EX_Reln-HIP_CA1_EX_Wfs1, HIP_CA1_EX_Wfs1-HIP_CA3_EX_Prkcd, HIP_CA3_EX_Prkcd-IN_Sst) # All must be DE.
```
...
anno_color <- hue_pal()(length(unique(sce$bio_reps)))
color3 <- colorRamp2(
...
anno <- HeatmapAnnotation(
  ...
  cell_type = anno_block(
    ...
    labels = c(levels(idents)[[1]], levels(idents)[[2]], levels(idents)[[3]]),
    labels_gp = gpar(col = "black", fontsize = 20)
  ),
  col = list(donor = c("B02" = anno_color[1], "B03" = anno_color[2])),
...
png(
  file.path("..", "..", "results", "01-all", "network.png"), 150,
  150, "in",
  res = 72
)
...
plot(
    ...
    edge.width = E(graph)$weight ^ 2 * 1.5,
    vertex.shape = "pie",
    ...
    vertex.label.cex = ifelse(
      DE == "yes", (all_DE_match$Freq + 5) * 2, 5
    ),
...
plot(
    ...
    vertex.label.font = vertex_label_font,
    vertex.label.cex = ifelse(
      DE == "yes" & membership >= module_membership_thresh, (all_DE_match$Freq + 3) * 4, 3
    ),
    vertex.frame.color = vertex_frame_color,
    vertex.label = vertex_label
)
