...
allen <- RenameIdents(allen, "23" = "EC_L2_Lat_Reln", "3" ="HIP_CA1_Wfs1", "29" = "HIP_CA3_Prkcd")
...
SpatialPlot(seurat, features = c("Reln", "Wfs1", "Prkcd", "Ptn"), pt.size.factor = 3.5)
```
...
SpatialPlot(
  seurat, features = c("EC-L2-Lat-Reln", "HIP-CA1-Wfs1", "HIP-CA3-Prkcd"),
  pt.size.factor = 3.5, alpha = c(0.1, 1)
)
```
...
seurat_sub <- suppressWarnings(
  seurat[
    rownames(seurat) == "EC-L2-Lat-Reln" | rownames(seurat) == "HIP-CA1-Wfs1" |
      rownames(seurat) == "HIP-CA3-Prkcd",
  ]
)
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), show_column_names = FALSE, cluster_rows = FALSE, column_split = 4, 
)
draw(heatmap)

seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[1:3])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 14
)
draw(heatmap)

seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[c(1, 3, 6, 7, 8, 9)])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 3
)
draw(heatmap)
...
idents[[4]] <- rep("NA", length(colnames(seurat)[colnames(seurat) %notin% colnames(seurat_sub)]))
...
seurat <- seurat[rowSums(as.matrix(GetAssayData(seurat, slot = "counts")) > 0) > 0, ]
seurat <- suppressWarnings(SCTransform(seurat, "Spatial", verbose = FALSE))
SpatialPlot(seurat, features = c("Reln", "Wfs1", "Prkcd", "Bok"), pt.size.factor = 3.5)
```
...
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 13,
  column_names_gp = gpar(fontsize = 4)
)
draw(heatmap)

seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[c(1:4, 8:11)])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 3,
  column_names_gp = gpar(fontsize = 4)
...
seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[c(1, 2, 5)])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 8,
  column_names_gp = gpar(fontsize = 4)
)
draw(heatmap)

seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[c(2, 4, 6)])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 3,
  column_names_gp = gpar(fontsize = 4)
...
ident_names <- c("EC_L2_Lat_Reln", "HIP_CA3_Prkcd", "HIP_CA1_Wfs1")
idents <- vector("list", 4)
for (i in 1:(length(idents) - 1)) {
  idents[[i]] <- rep(ident_names[i], length(column_order(heatmap)[[i]]))
}
...
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 12,
  column_names_gp = gpar(fontsize = 4)
)
draw(heatmap)

seurat_sub <- suppressWarnings(seurat_sub[ , unlist(column_order(heatmap)[c(1, 3, 6)])])
set.seed(1)
heatmap <- Heatmap(
  GetAssayData(seurat_sub), cluster_rows = FALSE, column_split = 3,
  column_names_gp = gpar(fontsize = 4)
...
ident_names <- c("HIP_CA3_Prkcd", "EC_L2_Lat_Reln", "HIP_CA1_Wfs1")
idents <- vector("list", 4)
for (i in 1:(length(idents) - 1)) {
  idents[[i]] <- rep(ident_names[i], length(column_order(heatmap)[[i]]))
}
