...
allen <- RenameIdents(allen, "23" = "EC Reln", "3" = "CA1 Wfs1", "29" = "CA3 Bok", "18" = "IN Sst")
...
SpatialPlot(
  seurat, features = c("EC Reln", "CA1 Wfs1", "CA3 Bok", "IN Sst"),
  pt.size.factor = 3.5, alpha = c(0.1, 1)
)
```
...
```{r}
seurat_sub <- suppressWarnings(seurat[rownames(seurat) == "EC Reln", ])
order <- order(GetAssayData(seurat_sub), decreasing = TRUE)
seurat_sub <- suppressWarnings(seurat_sub[ , c(order[2], order[3])])

ident_names <- "EC Reln"
idents <- vector("list", 2)
for (i in 1:(length(idents) - 1)) {
  idents[[i]] <- rep(ident_names[i], 2)
}
idents[[2]] <- rep("NA", length(colnames(seurat)[colnames(seurat) %notin% colnames(seurat_sub)]))

spot_names <- colnames(seurat_sub)
...
seurat_sub <- suppressWarnings(seurat[rownames(seurat) == "EC Reln", ])
order <- order(GetAssayData(seurat_sub), decreasing = TRUE)
seurat_sub <- suppressWarnings(seurat_sub[ , c(order[1], order[2])])
...
seurat_sub <- suppressWarnings(seurat[rownames(seurat) == "EC Reln", ])
order <- order(GetAssayData(seurat_sub), decreasing = TRUE)
seurat_sub <- suppressWarnings(seurat_sub[ , c(order[3], order[5])])
...
```{r}
seurat_sub <- suppressWarnings(seurat[rownames(seurat) == "EC Reln", ])
seurat_sub <- suppressWarnings(seurat_sub[ , which.max(GetAssayData(seurat_sub))])

ident_names <- "EC Reln"
idents <- vector("list", 2)
for (i in 1:(length(idents) - 1)) {
  idents[[i]] <- ident_names[i]
}
idents[[2]] <- rep("NA", length(colnames(seurat)[colnames(seurat) %notin% colnames(seurat_sub)]))
